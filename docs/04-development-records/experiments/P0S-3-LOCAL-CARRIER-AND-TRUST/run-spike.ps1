...
# NOT_PRODUCTION: disposable P0.S-3 evidence runner.
$ErrorActionPreference = 'Stop'
$utf8NoBom = [System.Text.UTF8Encoding]::new($false)
$experimentRoot = $PSScriptRoot
$runtimeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("shaco-forge-p0s3-{0}" -f [guid]::NewGuid().ToString('N'))
$profileRoot = Join-Path $runtimeRoot 'profiles\shaco-host'
$bundleRoot = Join-Path $profileRoot 'node_modules\@shaco-forge\not-production-p0s3-host-bundle'
...
$expectedShacoHead = '8308b406aff6248b620b9a6a62c66feb7d0aeeb4'
...
function Invoke-DshValidation {
  ...
  return [ordered]@{
    ...
    hasLocalBundle = $stdout.Contains('@shaco-forge/not-production-p0s3-host-bundle')
...
if (-not $IsWindows) { throw 'P0.S-3 requires real Windows Named Pipes and ACLs.' }
...
$pipeName = "shaco-forge-p0s3-$([guid]::NewGuid().ToString('N'))"
...
try {
  $workerProcess = Start-RedirectedProcess $pwshPath @('-NoLogo', '-NoProfile', '-NonInteractive', '-File', $workerScript) $experimentRoot @{
    P0S3_PIPE_NAME = $pipeName
    P0S3_EPHEMERAL_SECRET = $ephemeralSecret
    P0S3_WORKER_ID = $workerId
    P0S3_ENDPOINT_ID = $endpointId
    P0S3_USER_SID = $currentSid
    P0S3_WORKER_READY_FILE = $workerReadyFile
    P0S3_WORKER_SUMMARY_FILE = $workerSummaryFile
    P0S3_WORKER_FAILURE_FILE = $workerFailureFile
    P0S3_DSH_READY_FILE = $dshReadyFile
    P0S3_DSH_SUMMARY_FILE = $dshSummaryFile
    P0S3_DSH_FAILURE_FILE = $dshFailureFile
    P0S3_NODE_PATH = $nodePath
    P0S3_DSH_CLI = $cliBin
    P0S3_RUNTIME_ROOT = $runtimeRoot
    P0S3_WORKSPACE_ROOT = $workspaceRoot
  }
  ...
  $electronEnvironment = @{
    ...
    P0S3_ENDPOINT_ID = $endpointId
    P0S3_USER_SID = $currentSid
    P0S3_WORKER_PROCESS_ID = $workerProcess.Id
    P0S3_DSH_PROCESS_ID = $workerReady.dshPid
    P0S3_ELECTRON_RESULT_FILE = $electronResultFile
    P0S3_ELECTRON_USER_DATA = (Join-Path $runRoot 'electron-user-data')
  ...
  if (-not $electronProcess.WaitForExit(180000)) {
  ...
  $resultPass = ($electronResult.result -eq 'PASS') -and
    ...
    ($workerSummary.streamForwardedCount -eq $dshSummary.streamProbeOpenCount) -and
  ...
  $summary = [ordered]@{
    ...
    runtimeRoot = '<TEMP>\shaco-forge-p0s3-<RUN_ID>'
    baseline = [ordered]@{
    ...
    gates = [ordered]@{
      ...
      unaryHarnessGateway = $electronResult.positive.unary.pass
      concurrentUnary = $electronResult.positive.concurrentUnary.pass
      basicStreamConcurrency = $electronResult.positive.mixedUnaryAndStreams.pass
      partialFrameAssembly = $workerSummary.framesRequiringMultipleReads -gt 0 -and $electronResult.framing.clientParser.parserPartialWaits -gt 0
      payloadMatrix = $electronResult.positive.payloads.pass
