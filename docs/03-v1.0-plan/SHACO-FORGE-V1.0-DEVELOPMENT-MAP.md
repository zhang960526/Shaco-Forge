...
共同构成最终 Step2 review chain。Architecture Owner 已接受该 chain、关闭并冻结 Step2，
并仅授权 Step3 为 not started。
...
Step3 保持 Frozen Parent Contract + effective Release Trust Amendment scope。新增 durability
obligation 只要求归档 final immutable RC package bytes 和可复核 Evidence，不建立新的架构服务。
Provider/Signing 仍未授权；Slice4 Fresh Windows acceptance 未开始。
