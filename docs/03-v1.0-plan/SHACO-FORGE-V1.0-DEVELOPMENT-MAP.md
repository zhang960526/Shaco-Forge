...
冻结 [Release Trust / Signing Amendment](V1-SLICE-3-RELEASE-TRUST-SIGNING-REQUIREMENT-AMENDMENT-CANDIDATE.md)，
使 `GITHUB_OPEN_SOURCE_UNSIGNED` 成为 active V1 release trust mode，并只授权下一批最小
Product Source Corrective；该 Corrective 尚未开始。
...
Current route 仅允许下一批执行 frozen Amendment 的 release-trust policy delta。此授权不
关闭 Step2，不授权 Step3/Provider/Signing。Step3 packaged cumulative acceptance 与
Slice4 Fresh Windows unsigned-release acceptance 保持。
