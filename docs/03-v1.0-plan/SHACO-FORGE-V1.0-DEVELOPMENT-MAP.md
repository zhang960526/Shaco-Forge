...
Frozen [Release Trust / Signing Amendment](V1-SLICE-3-RELEASE-TRUST-SIGNING-REQUIREMENT-AMENDMENT-CANDIDATE.md)
已由最小 [Product Source Corrective](../04-development-records/V1-SLICE-3-RELEASE-TRUST-CORRECTIVE-IMPLEMENTATION-RECORD.md)
实现。Active V1 mode 为 immutable `GITHUB_OPEN_SOURCE_UNSIGNED`，同时保留并回归验证
`TRUSTED_AUTHENTICODE`；23 项最终 gate 在同一源码清单上全部通过。
...
STEP2_SIGNING_ACCEPTANCE = SIGNING_INTEGRATION_VERIFIED + GITHUB_UNSIGNED_RELEASE_POLICY_VERIFIED + INSTALLER_SHA256_AND_RELEASE_MANIFEST_BOUND
...
Current route 仅允许独立审查本 Corrective。Step2 未关闭/未冻结，Step3、Provider、Signing
均未授权；Slice4 Fresh Windows unsigned-release acceptance 仍未开始。
