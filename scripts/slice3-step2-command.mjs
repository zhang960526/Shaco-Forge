// Step2 build/test ledger. Never executes Provider or production signing.
...
const allowed = ['verify:f05', 'diagnose:slice3-step2', 'test:slice3-step2', 'package:installer', 'test:installer', 'smoke:slice3-step2', 'typecheck', 'build', 'test', 'verify:static', 'verify:theme', 'smoke:worker', 'smoke:carrier', 'smoke:electron', 'smoke:failure', 'smoke:slice2-step1', 'smoke:slice2-step2', 'smoke:slice2-step3', 'smoke:slice2-step3-interactions', 'test:full-shaco', 'smoke:full-shaco', 'package:runtime', 'test:packaged-runtime', 'smoke:packaged-runtime', 'packaged-startup-probe', 'composition', 'install-packager']
assert.ok(allowed.includes(command) || ['packaged-desktop-diagnostic', 'packaged-worker-diagnostic', 'packaged-diagnostic-cleanup'].includes(command), 'Command outside Step2 non-Provider allowlist')
...
summary.commands.push({ command, phase: process.env.SHACO_FORGE_SLICE3_STEP2_FINAL === '1' ? 'FINAL_REGRESSION' : 'DEVELOPMENT',
