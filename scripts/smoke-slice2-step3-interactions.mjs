...
import { verifyFrozen } from './step3-final-composition.mjs'

assert.equal(process.version, 'v22.19.0')
await verifyFrozen()
const root = await mkdtemp(join(paths.root, 'node_modules/.review026-interactions-'))
...
async function desktop(index) {
  const child = spawn(electron, [`--user-data-dir=${join(root, 'desktop-' + index)}`, join(paths.desktopRoot, 'test-fixtures/step3-interactions-main.mjs')], {
    env: { ...process.env, SHACO_FORGE_WORKER_NODE:process.execPath, SHACO_FORGE_WORKER_ENTRY:paths.workerEntry,
      ...
      SHACO_FORGE_STEP2_WORKSPACE:workspace }, windowsHide:true, stdio:['ignore','pipe','pipe','ipc'],
  })
  children.push(child)
  child.stdout.resume()
  let diagnostics = ''
...
try {
  ...
  await evidence('s3g16-results.json',{result,failure,failureDetails,gates,cleanup,desktopExits,providerRuns:0})
