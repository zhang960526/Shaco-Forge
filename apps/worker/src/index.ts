...
import { assertP8OverlayPreflight, cleanupP8Overlay, createP8Overlay, type P8Overlay } from './p8-overlay.js'
...
let p8Overlay: P8Overlay | undefined
const workerInstanceId = randomUUID()

function emit(phase: BootstrapPhase, extra: Partial<BootstrapEvent> = {}): BootstrapEvent {
...
async function stop(exitCode = 0): Promise<void> {
  ...
  let overlayCleanupError: string | undefined
  if (p8Overlay !== undefined) {
    try { await cleanupP8Overlay(p8Overlay); p8Overlay = undefined }
    catch (error) { overlayCleanupError = error instanceof Error ? error.message : String(error) }
  }
  emit('worker-stopped', { hostPid: host?.pid, exitCode: overlayCleanupError === undefined ? exitCode : 1, error: overlayCleanupError })
  process.exit(overlayCleanupError === undefined ? exitCode : 1)
}
...
async function startWorkerInner(providedConfig?: import('./config.js').WorkerConfig): Promise<void> {
  ...
  const config = providedConfig ?? await readPackagedWorkerConfig(sourceDir) ?? readWorkerConfig(process.env, process.argv.slice(2))
  ...
  if (config.p8Activation !== undefined && config.packagedRoot === undefined) throw new Error('P8_ACCEPTANCE_PACKAGED_ONLY')
  if (process.argv.includes('--preflight')) {
    if (config.p8Activation !== undefined) {
      const runtime = packagedHarnessRuntime(config.packagedRoot!)
      await assertP8OverlayPreflight(config.dshHome, runtime.overlayNodeModules)
    }
    process.stdout.write(JSON.stringify({ result: 'PASS', writes: 0, dshHome: config.dshHome,
      p8Activation: config.p8Activation }) + '\n')
    return
  }
  ...
  if (config.p8Activation !== undefined) {
    p8Overlay = await createP8Overlay(config.dshHome, runtime.overlayNodeModules, config.p8Activation)
  }
  ...
  const hostLaunchArgv = [runtime.cliPath, '--profile', config.profileName,
    ...(p8Overlay === undefined ? [] : ['--patch', p8Overlay.path])]
  ...
  let hostReadyEvidence: BootstrapEvent | undefined
  let stdoutBuffer = ''
  ...
  hostStdout.on('data', (chunk: string) => {
    ...
    for (const line of lines) {
      try {
        ...
        if (config.p8Activation !== undefined && (
          value.P8_ENABLED !== true
          || value.AUTHORIZATION_ID !== config.p8Activation.authorizationId
          || value.SCENARIO_ID !== config.p8Activation.scenarioId
          || value.SCENARIO_BUDGET_INITIAL !== config.p8Activation.modelDispatchBudget
          || value.SCENARIO_BUDGET_REMAINING !== config.p8Activation.modelDispatchBudget
          || value.PROVIDER !== 'deepseek-official'
          || value.MODEL !== 'deepseek-v4-flash'
          || value.INPUT_CLASS !== 'text-only'
          || value.RESOLVED_RETRY_MODE !== 'normal'
          || value.RESOLVED_MAX_RETRIES !== config.p8Activation.maxRetries
        )) throw new Error('P8_HOST_EFFECTIVE_STATE_MISMATCH')
        if (config.p8Activation === undefined && value.P8_ENABLED === true) {
          throw new Error('P8_HOST_EFFECTIVE_STATE_UNEXPECTED')
        }
        hostWasReady = true
        hostReadyEvidence = emit('host-ready', {
          hostPid: host?.pid,
          hostLaunchArgv: [process.execPath, ...hostLaunchArgv],
          profileName: config.profileName,
          profilePath,
          ...
          harnessCliPath: runtime.cliPath,
          harnessSourceRoot: config.harnessRoot,
          readiness: { ...value, gatewayPresetCount: value.gatewayPresetCount, standardPresetPresent: true },
          p8OverlaySha256: p8Overlay?.sha256,
        })
        hostReadyResolve()
      } catch (error) {
  ...
  if (hostReadyEvidence === undefined) throw new Error('Host readiness evidence is unavailable')
  const helperReady = readOneFrame(helperControl, 10_000)
  await writeFrame(helperInput, { type: 'authority-ready', hostPreflight: {
    ...hostPreflight,
    bootstrapEvidence: { workerInstanceId, hostReady: hostReadyEvidence },
  } })
...
async function cleanupFailedP8Start(): Promise<void> {
  if (p8Overlay === undefined) return
  for (const child of [helper, host]) {
    if (child !== undefined && child.exitCode === null && child.signalCode === null) child.kill('SIGTERM')
  }
  await Promise.race([
  ...
  await cleanupP8Overlay(p8Overlay)
  p8Overlay = undefined
}

export async function startWorker(providedConfig?: import('./config.js').WorkerConfig): Promise<void> {
  try { await startWorkerInner(providedConfig) }
  catch (error) {
    try { await cleanupFailedP8Start() }
    catch (cleanupError) {
      throw new AggregateError([error, cleanupError], 'P8 startup failed and overlay cleanup was rejected')
    }
    throw error
  }
}
