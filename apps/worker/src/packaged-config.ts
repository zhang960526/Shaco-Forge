...
import { parseP8ActivationArgs } from '@shaco-forge/contracts/p8-activation'
...
export async function readPackagedWorkerConfig(sourceDir: string, argv: readonly string[] = process.argv.slice(2)): Promise<WorkerConfig | undefined> {
  ...
  const p8Activation = parseP8ActivationArgs(argv)
  if (process.argv.includes('--preflight')) return { dshHome: control.dshHome, harnessRoot: join(root, 'harness'), nativeHelperPath, profileName: 'shaco-forge', packagedRoot: root, p8Activation }
  ...
  return { dshHome: home.dshHome, harnessRoot: join(root, 'harness'), nativeHelperPath, profileName: 'shaco-forge', packagedRoot: root, p8Activation }
}
