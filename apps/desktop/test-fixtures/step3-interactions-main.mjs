...
import { captureVisual, verifyThemeSwitch, verifyControls, captureFamicom, verifyFamicomPresentation, verifyBraunPresentation } from './full-shaco-visual-driver.mjs'
...
async function command(action, payload) {
  ...
  if (action === 'famicom-capture') return captureFamicom(window, payload.name)
  if (action === 'famicom-check') return verifyFamicomPresentation(window, payload.stage)
  if (action === 'braun-check') return verifyBraunPresentation(window, payload.stage)
