interface BootstrapProjection {
  ...
  lifecycleState?: 'RUNNING' | 'STOPPING' | 'STOPPED' | 'STOP_FAILED'
}
...
interface Window {
  ...
  shacoForge: {
    lifecycle: { stopWorker(): Promise<void>; exitProduct(): Promise<void> }
    workspace: { pickDirectory(): Promise<string | null> }
    appearance: { setTitleBar(resolved: 'light' | 'dark', template: 'BRAUN' | 'FAMICOM'): Promise<void> }
    bootstrap: {
