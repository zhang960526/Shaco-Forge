...
import { bindWorkerMenu } from './worker-menu.js'
...
const syncTitleBar = (): void => {
  const theme = themeController.getSnapshot()
  void window.shacoForge.appearance.setTitleBar(theme.resolved, theme.template)
}
const unsubscribeTitleBar = themeController.subscribe(syncTitleBar)
syncTitleBar()
...
const workerMenu = bindWorkerMenu(requiredElement<HTMLElement>('.worker-controls'), window.shacoForge.lifecycle)
...
function displayState(): void {
  const connected = runResolved && !failed && projection?.connectionState === 'CONNECTED' && (!projection.lifecycleState || projection.lifecycleState === 'RUNNING')
  ...
  if (!connected) {
    ...
    detail.textContent = projection?.lifecycleState === 'STOPPED' ? 'Worker 已停止。重新打开 Shaco Forge 后可继续工作。'
      : projection?.lifecycleState === 'STOPPING' ? '正在停止 Worker…'
      : projection?.lifecycleState === 'STOP_FAILED' ? 'Worker 停止失败，工作空间已断开。请确认状态后重试。'
      : failed ? '工作空间暂不可用，请重新打开窗口。' : projection?.connectionState === 'RECONNECTING' || projection?.connectionState === 'CONNECTION_LOST' ? '连接已断开，正在重新连接…' : '正在准备工作空间…'
...
window.addEventListener('pagehide', () => {
  unsubscribeBootstrap(); unsubscribeTitleBar(); workerMenu.dispose(); themeController.dispose(); void entry?.dispose()
}, { once: true })
