...
export function createShellState(ctx, native) {
  ...
  return {
    ...
    manageSession: (kind, workspaceId, sessionId) => {
      ...
      if (!session || session.blank || !['rename-session', 'archive-session'].includes(kind)) return
    ...
    submitDialog: () => {
      ...
      return mutation(async token => {
        ...
        if (dialog.kind === 'rename-session') {
          ...
          let row = ctx.sessions.list.getSnapshot().byId[dialog.sessionId]
          if (!row || row.displayTitle !== result.value.title) {
            try { await ctx.sessions.refresh() }
            catch { throw new Error('OUTCOME_UNKNOWN') }
            check(token)
            if (ctx.sessions.binding(dialog.sessionId) !== binding) throw new Error('STALE_GENERATION')
            row = ctx.sessions.list.getSnapshot().byId[dialog.sessionId]
          }
    ...
    newChatIn: workspaceId => operation(token => connect(workspaceId, token)),
