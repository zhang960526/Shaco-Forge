...
export function TurnDisclosure({ chat, turn }) {
  ...
  return h('details', { className: 'shaco-tool shaco-turn-details', 'data-testid': containsTool ? 'shaco-tool' : undefined, 'data-turn-details': String(turn.turn) },
    h('summary', null, h('span', { className: 'state-dot', 'data-state': projected.duration.state === 'completed' ? 'settled' : projected.duration.state }),
      h('strong', null, summary), h('span', { className: 'tool-summary' }, '详情')),
    h('ol', { className: 'turn-detail-list', 'aria-label': `第 ${turn.turn} 轮执行详情` }, projected.entries.length === 0
      ? h('li', { 'data-detail-kind': 'empty' }, '暂无可用步骤')
      : projected.entries.map((entry, index) => h('li', { key: index, 'data-detail-kind': entry.kind,
        'data-tool-state': entry.kind === 'tool' || entry.kind === 'subagent' ? entry.state : undefined,
        style: entry.depth ? { paddingInlineStart: `${entry.depth * 16}px` } : undefined },
      h('span', null, entry.label), entry.attempt !== undefined && h('span', null, ` ${entry.attempt}`),
