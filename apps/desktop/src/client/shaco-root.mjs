...
function ShacoShell({ ctx, shell }) {
  ...
  return h('div', { className: 'app-shell', 'data-collapsed': collapsed, 'data-shaco-shell': '', 'data-testid': 'shaco-root' },
    h(Sidebar, { shell, workspaces: ctx.workspaces.list, sessions: ctx.sessions.list, collapsed, toggleSidebar: () => setCollapsed(value => !value) }),
    h('main', { className: 'workspace' },
      h('header', { className: 'shaco-workbar' },
        h('div', null, h('small', null, project?.title ?? '项目工作空间'), h('h1', null, chat?.displayTitle ?? '新对话')),
        // Main's lifecycle projection owns the persistent Worker controls outside this disposable root.
        h('div', { className: 'worker-controls-space', 'aria-hidden': true })),
