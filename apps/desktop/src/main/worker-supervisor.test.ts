...
import { assertAutomaticWorkerReplacementAllowed, assertWorkerActivationMatches, mapWorkerTerminal, readSupervisorConfig,
  PACKAGED_PREFLIGHT_TIMEOUT_MS, workerInvocationArguments, WorkerSupervisor, WorkerTerminationState, type SupervisorConfig } from './worker-supervisor.js'
...
test('packaged preflight allows a bounded cold-cache verification window', () => {
  assert.equal(PACKAGED_PREFLIGHT_TIMEOUT_MS, 180_000)
})

test('normal launch arguments remain unchanged and P8 launch carries only the normalized tuple', () => {
  const normal = fixtureConfig('shaco-forge')
  assert.deepEqual(workerInvocationArguments(normal), [normal.workerEntryPath])
  assert.deepEqual(workerInvocationArguments(normal, true), [normal.workerEntryPath, '--preflight'])

  const p8 = { ...normal, p8Activation: {
    authorizationId: 'AUTH-01', scenarioId: 'S-B01', modelDispatchBudget: 3, maxRetries: 1 as const,
  } }
  assert.deepEqual(workerInvocationArguments(p8), [normal.workerEntryPath, '--p8-acceptance',
    '--p8-authorization-id=AUTH-01', '--p8-scenario-id=S-B01', '--p8-model-dispatch-budget=3', '--p8-max-retries=1'])
  assert.doesNotThrow(() => assertWorkerActivationMatches(p8, workerInvocationArguments(p8)))
  assert.throws(() => assertWorkerActivationMatches(normal, workerInvocationArguments(p8)), /IDENTITY_MISMATCH/)
})

test('P8 Worker loss terminates the scenario instead of authorizing a budget refill', () => {
  const p8 = { ...fixtureConfig('shaco-forge'), p8Activation: {
    authorizationId: 'AUTH-LOSS-01', scenarioId: 'S-B03', modelDispatchBudget: 2, maxRetries: 0 as const,
  } }
  assert.throws(() => assertAutomaticWorkerReplacementAllowed(p8), /P8_SCENARIO_TERMINATED/)
  assert.doesNotThrow(() => assertAutomaticWorkerReplacementAllowed(fixtureConfig('shaco-forge')))
})
