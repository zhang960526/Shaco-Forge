...
test('generation 1 startup failures are not classified as reconnect failures', async () => {
  for (const [message, expected] of [
    ['WORKER_START_TIMEOUT: packaged preflight exceeded 180000 ms', 'WORKER_START_TIMEOUT'],
    ['WORKER_AUTHORITY_FAILURE: startup exited', 'WORKER_AUTHORITY_FAILURE'],
  ]) {
    const supervisor = { start: async (): Promise<CarrierBootstrap> => { throw new Error(message) },
      recover: async (): Promise<CarrierBootstrap> => { throw new Error('not called') }, carrierReady() {}, detach() {} }
    const coordinator = new RecoveryCoordinator(supervisor, () => {}, async () => {})
    await coordinator.start()
    assert.equal(coordinator.projection.generation, 1)
    assert.equal(coordinator.projection.failure, expected)
    assert.notEqual(coordinator.projection.failure, 'RECONNECT_FAILED')
    coordinator.stop()
  }
})
...
test('an actual reconnect failure remains RECONNECT_FAILED', async () => {
  const { coordinator: c, setRecover } = setup()
  try {
    await c.start()
    setRecover(async () => { throw new Error('replacement launch failed') })
    c.lost('WORKER_CRASHED'); await c.settled()
    assert.equal(c.projection.generation, 2)
    assert.equal(c.projection.connectionState, 'FAILED')
    assert.equal(c.projection.failure, 'RECONNECT_FAILED')
  } finally { c.stop() }
})
