...
import { assertLifecycleDocument, ProductLifecycle, type ProductLifecycleState } from './product-lifecycle.js'
...
let lifecycle: ProductLifecycle | undefined
...
ipcMain.handle('appearance:set-title-bar', (event, input: unknown) => {
  assertTrustedRenderer(event)
  if (!isRecord(input) || !['light', 'dark'].includes(String(input.resolved)) || !['BRAUN', 'FAMICOM'].includes(String(input.template))) {
    throw new TypeError('Invalid title bar appearance')
  }
  const resolved = input.resolved as 'light' | 'dark'
  const template = input.template as 'BRAUN' | 'FAMICOM'
  const palette = template === 'BRAUN'
    ? resolved === 'dark' ? { color: '#252622', symbolColor: '#eeeee8' } : { color: '#eeeee8', symbolColor: '#272a27' }
    : { color: '#00000000', symbolColor: resolved === 'dark' ? '#f9edd6' : '#fffaf1' }
...
function registerTransportIpc(): void {
  const currentLifecycle = (event: IpcMainInvokeEvent, generation: unknown, epoch: unknown) => {
    const assertCurrent = () => {
      assertTrustedRenderer(event)
      assertLifecycleDocument(event, mainWindow, generation, epoch, recovery?.projection.generation, documentEpoch)
    }
    assertCurrent()
    if (!lifecycle) throw new Error('WORKER_LIFECYCLE_UNAVAILABLE')
    return { lifecycle, assertCurrent }
  }
  ipcMain.handle('lifecycle:stop-worker', async (event, generation: unknown, epoch: unknown) => {
    const current = currentLifecycle(event, generation, epoch)
    await current.lifecycle.stopWorker(current.assertCurrent)
  })
  ipcMain.handle('lifecycle:exit-product', async (event, generation: unknown, epoch: unknown) => {
    const current = currentLifecycle(event, generation, epoch)
    await current.lifecycle.exitProduct(current.assertCurrent)
  })
...
async function createMainWindow(): Promise<void> {
  mainWindow = new BrowserWindow({
    ...
    minWidth: 760,
    minHeight: 600,
    icon: resolve(currentDir, 'app-icon.png'),
    autoHideMenuBar: true,
    titleBarStyle: 'hidden',
    titleBarOverlay: { color: '#eeeee8', symbolColor: '#272a27', height: 54 },
...
ipcMain.handle('bootstrap:get-state', () => ({ ...projection, documentEpoch }))
...
export async function startDesktop(providedConfig?: SupervisorConfig): Promise<void> {
  ...
  try {
    ...
    lifecycle = new ProductLifecycle(recovery, supervisor, state => {
      publishProjection({ ...projection, lifecycleState: state, connectionState: 'DISCONNECTED', projectionRebuildComplete: false,
        phase: state === 'STOPPED' ? 'worker-stopped' : state === 'STOPPING' ? 'worker-stopping' : 'worker-stop-failed',
        message: state === 'STOPPED' ? 'Worker Stopped' : state === 'STOPPING' ? 'Worker Stopping' : 'Controlled Worker Stop Failed' })
    }, () => app.quit())
