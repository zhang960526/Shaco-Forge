...
function classifyAttachmentFailure(text: string, state: ConnectionState, recover: boolean): string {
  const mismatch = (Object.keys(MISMATCH_ACTIONS) as MismatchCode[]).find(code => text === code || text.startsWith(code + ':'))
  if (mismatch !== undefined) return mismatch + ': ' + MISMATCH_ACTIONS[mismatch]
  if (/version|protocol|incompatible/i.test(text)) return 'WORKER_VERSION_INCOMPATIBLE'
  if (/AMBIGUOUS|IDENTITY|prior death/.test(text)) return 'AUTHORITY_AMBIGUOUS_FAIL_CLOSED'
  if (state === 'AUTHENTICATING') return 'CARRIER_AUTH_FAILED'
  if (recover) return 'RECONNECT_FAILED'
  if (/^WORKER_START_TIMEOUT(?::|$)|timed? out|timeout/i.test(text)) return 'WORKER_START_TIMEOUT'
  return 'WORKER_AUTHORITY_FAILURE'
}
  ...
  async #attach(recover: boolean): Promise<void> {
    ...
    try {
      ...
      if (!this.#stopped && this.#attempt === attempt && this.projection.generation === generation) {
        ...
        this.fail(classifyAttachmentFailure(text, this.projection.connectionState, recover))
      }
      candidate?.close()
    }
  }
