...
import { p8ActivationArgs, parseP8ActivationArgs, sameP8Activation, type P8Activation } from '@shaco-forge/contracts/p8-activation'

export const WORKER_STARTUP_TIMEOUT_MS = 90_000
export const PACKAGED_PREFLIGHT_TIMEOUT_MS = 180_000
...
export interface SupervisorConfig {
  ...
  dshHome: string
  profileName: string
  p8Activation?: P8Activation
}
export async function readPackagedSupervisorConfig(
  root: string,
  ...
  return { packagedRoot: root, workerNodePath: releasePath(root, RELEASE_LAYOUT.node),
    ...
    p8Activation: parseP8ActivationArgs(argv) }
}
...
export function workerInvocationArguments(config: SupervisorConfig, preflight = false): string[] {
  return [config.workerEntryPath, ...(preflight ? ['--preflight'] : []), ...p8ActivationArgs(config.p8Activation)]
}

export function assertWorkerActivationMatches(config: SupervisorConfig, workerArgv: readonly string[]): void {
  let actual: P8Activation | undefined
  try { actual = parseP8ActivationArgs(workerArgv) }
  catch { throw new Error('P8_WORKER_ACTIVATION_IDENTITY_MISMATCH') }
  if (!sameP8Activation(config.p8Activation, actual)) throw new Error('P8_WORKER_ACTIVATION_IDENTITY_MISMATCH')
}
export function assertAutomaticWorkerReplacementAllowed(config: SupervisorConfig): void {
  if (config.p8Activation !== undefined) throw new Error('P8_SCENARIO_TERMINATED: automatic authority replacement forbidden')
}
...
export class WorkerSupervisor {
  ...
  async start(): Promise<CarrierBootstrap> {
    ...
    if (release) {
      try {
        const preflight = await promisify(execFile)(this.config.workerNodePath, workerInvocationArguments(this.config, true), {
          env: workerEnvironment(this.config), windowsHide: true, timeout: PACKAGED_PREFLIGHT_TIMEOUT_MS,
        })
        const result = JSON.parse(preflight.stdout) as { dshHome: string; p8Activation?: P8Activation }
        attestedDshHome = result.dshHome
        if (!sameP8Activation(this.config.p8Activation, result.p8Activation)) throw new Error('P8_PREFLIGHT_ACTIVATION_IDENTITY_MISMATCH')
      } catch (error) {
        ...
        if ((error as { killed?: boolean }).killed === true) {
          throw new Error(`WORKER_START_TIMEOUT: packaged preflight exceeded ${PACKAGED_PREFLIGHT_TIMEOUT_MS} ms`)
        }
    ...
    try { this.#status = await this.discover() }
      ...
      const child = spawn(this.config.workerNodePath, workerInvocationArguments(this.config), {
    ...
    assertWorkerActivationMatches(this.config, this.#status!.workerRuntime.argv)
  ...
  async recover(): Promise<CarrierBootstrap> {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        assertAutomaticWorkerReplacementAllowed(this.config)
